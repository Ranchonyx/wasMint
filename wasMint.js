...
class wasMintModule {
    ...
    constructor(wasmPath, importConfig = {
        ...
        (async () => {
            await WebAssembly.instantiateStreaming(fetch(wasmPath), importConfig).then(obj => {
                ...
                if(this.wasMintHasSignature("malloc") && this.wasMintHasSignature("free") && this.wasMintHasSignature("memory")) {
                ...
                this.wasMintHasSignature("main") ? this.potentialEntryPoint = this.exports.main : undefined;
                        ...
                        this.functions[funKey].call = (...primaryArgs) => {
                            ...
                            for(let i = 0; i < this.functions[funKey].params.length; i++) {
                                if(_typeof(primaryArgs[i]) !== this.functions[funKey].params[i]) {
                                    throw new Error(`Invalid parameter type of [${_typeof(primaryArgs[i])}] instead of [${this.functions[funKey].params[i]}] at [${i}] for [${funKey}]`);
                                }
                            }
                            ...
                            if(_typeof(returnLength) !== "Number") {
                                ...
                                if(returnLength.length > 2 && returnLength.startsWith("M")) {
                                    ...
                                    strArgsToIndices.forEach(e => returnLength = returnLength.replace(e[0], finalArgs[e[0].split('')[1]]));
                                    returnLength = returnLength.replaceAll(' ', '').match(/^([+\-]?\d+(\.\d+)?([+\-*/]\d+(\.\d+)?)*|[a-zA-Z][a-zA-Z0-9]*([+\-*/][a-zA-Z][a-zA-Z0-9]*)*)$/g);
                                    returnLength = Function(`"use strict";return ${returnLength};`)();

                                } else if(returnLength.length > 2 && returnLength.startsWith("J")) {
                            ...
                            if(this.functions[funKey].return.type === "Void") {
                                ...
                                throw new Error(`Invalid return type configuration of [${_typeof(finalResult)}] instead of [${this.functions[funKey].return.type}] for [${funKey}]`);
                            } else {
                                return finalResult;
                            }
                        }
